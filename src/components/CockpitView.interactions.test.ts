...
describe("CockpitView interactions", () => {
  ...
  it("renders the hotspot layer as HTML elements positioned by checklist coordinates", async () => {
    ...
    expect(hotspots).toHaveLength(
      getPhaseById("cockpit-prep")!.items.length - 1,
    );
  ...
  it("pans by updating the viewport scroll position during drag", async () => {
    ...
    expect(wrapper.find(".cockpit-viewport").classes()).toContain(
      "is-dragging",
    );

    window.dispatchEvent(new MouseEvent("mouseup"));
    await nextTick();
    expect(wrapper.find(".cockpit-viewport").classes()).not.toContain(
      "is-dragging",
    );
    wrapper.unmount();
  });
  ...
  it("opens the modal when a hotspot is clicked", async () => {
    ...
    expect(wrapper.find(".modal-card h2").text()).toBe(
      getPhaseById("cockpit-prep")!.items[1].item,
    );
    wrapper.unmount();
  });
  ...
  it("supports dev mode coordinate capture and closest item detection", async () => {
    ...
    expect(panel.text()).toContain("Clicked: x: 25, y: 30");
    // Near Fuel Quantity (45.98, 62.32) - 25,30 is too far (> 1%)
    expect(panel.text()).toContain("No item within 1% range");

    // Click near cp2 (45.98, 62.32)
    // imageWrapper is 2000x1000 at 100,100
    // x = 45.98% of 2000 = 919.6 + 100 = 1019.6
    // y = 62.32% of 1000 = 623.2 + 100 = 723.2
    await wrapper.find(".image-wrapper").trigger("click", {
      clientX: 1020,
      clientY: 723,
    });
    expect(panel.text()).toContain("Closest: Fuel Quantity (cp2)");
